...
public class rzucakProvider implements InventoryProvider {
    @Override
    public void init(Player player, InventoryContents contents) {
        ...
        ItemStack stone = new ItemStack(Material.TNT, 64);
        ItemStack piston = new ItemStack(Material.TNT, 64);
        ...
        contents.set(2, 6, ClickableItem.empty(MainCraftingsProvider.rtnt)); //efekt
        ...
        contents.set(5, 6, ClickableItem.of(ac, e ->{
            String it = Config.COST_TNT;
            List<ItemStack> items = ItemUtil.getItems(it, 1);
            player.closeInventory();
            if (!ItemUtil.checkAndRemove(items, player)) {
                Utill.sendMsg(player, "&4Blad: &cPotrzebujesz&8: &4TNT&8(&e288&8)&7.");
                return;
            }
            ItemStack boy = MainCraftingsProvider.rtnt;
            player.getInventory().addItem(boy);
            Utill.sendMsg(player, "&7Udalo ci sie stworzyc &3Rzucane TNT&7!");
