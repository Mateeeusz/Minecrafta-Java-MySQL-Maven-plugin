...
public class enderProvider implements InventoryProvider {
    @Override
    public void init(Player player, InventoryContents contents) {
        ...
        ItemStack piston = new ItemStack(Material.ENDER_PEARL);
        ItemStack boy = new ItemStack(Material.ENDER_CHEST);
        ...
        contents.set(2, 6, ClickableItem.empty(boy)); //efekt
        ...
        contents.set(5, 6, ClickableItem.of(ac, e ->{
            String it = Config.COST_Enderchest;
            List<ItemStack> items = ItemUtil.getItems(it, 1);
            player.closeInventory();
            if (!ItemUtil.checkAndRemove(items, player)) {
                Utill.sendMsg(player, "&4Blad: &cPotrzebujesz&8: &3Obsydian&8(&e8&8)&7, &3Perla&8(&e1&8)&7.");

                return;
            }
            player.getInventory().addItem(boy);
            Utill.sendMsg(player, "&7Udalo ci sie stworzyc &3enderchesta&7!");
